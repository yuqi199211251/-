...
(function() {
    ...
    .controller("loginCtrl",[ "$scope","$rootScope",'$route','$location', "jmService","localStorageService",
					function($scope,$rootScope, $route,$location, apiSvc,cacheSvc) {
						$scope.login = {
							username : "",
							password : "",
							domain: "JWMS"
						...
						$scope.submitForm = function() {
							apiSvc.login($scope.login).$promise.then(function(data) {
                                if (data.loginUser){
									//$rootScope.FUserID = data.loginUser.FUserID;
