...
const sqlSvc=require("./sqlService");
  ...
  exports.getMonthlySalesDifferencesComparisonList=function(p_UserID,iEmpId,UserType,productTypeName,queryYear,queryMonth){
    let stmt=["exec Proc_SaleMonthlyDiff"];
   
    stmt.push(`${queryYear},`),
    stmt.push(`${queryMonth},`),
    stmt.push(`'初稿',`),
    stmt.push(`'终稿版',`),
  ...
  exports.getComplaintRequest=function(FID){
    ...
    stmt += " a.F6_8 as balloonCatheter,a.F6_9 as SupportHead,a.F6_10 as leadWire,a.F6_11 as OtherEvaluationContentsDetails,a.F6_12 as isCd,a.F6_13 as doctorSuggests,a.F6_14 as otherNote,"
    stmt += " a.F7_1 as clinicalOutcome,a.F7_2 as patientInjury,a.F7_3 as patientComplication,a.F7_4 as complicationDetail,a.F7_5 as selectedSite,a.F7_6 as isSurgery,a.F7_7 as incidentDescription,"
    stmt += " a.FYY as hospitalId,a.FFXS as distributorId,a.fdzzs as electronicCertificate,a.F3_4 as email,ac.FShdd,ac.FYb,ac.FShr,ac.FTel,ac.FPhone,a.t_DJLB as ProductTypeId,wdh.fdzzs as fdzzsUrl From a_Fkbg a left join a_Cust ac on ac.FCustID = a.FFXS left join WWW_DZZS_HH wdh on wdh.fbillno = a.FID  WHERE a.FID = @FID "
    let paramTypes={};
    let paramValues={};
  ...
  exports.getHospitalMessage=function(p_UserID,hospitalId){
    ...
    var stmt = "Select distinct ve.FCustID,ve.FNumber,ve.FItemID as FInterId,ve.FName,ve.FYydj as FYyDj,ve.FFxsID,ve.FFxsName,toz.FItemID From";
    stmt += " view_EmpCust ve left join t_BOSPTEntry2 tbp ON tbp.FJXSID = ve.FCustID and Getdate() between tbp.FbeginDate and tbp.FendDate ";
  ...
  exports.getCertificateList=function(p_UserID,hospitalId){
    ...
    stmt += " inner join view_EmpCust ve on ve.FFxsID = too.FitemID  Where ve.FEmpID = (SELECT FEmpID FROM t_User WHERE FUserID = @p_UserID) and ve.FItemID=@hospitalId ";
    let paramTypes={};
    let paramValues={};
    paramTypes["p_UserID"] = 'sql.Int';
    paramValues["p_UserID"] = p_UserID;
  ...
  exports.getSFE_ImplantData = function(date,productType,positionName,sUserType,FEmpID){
    date = parseInt(date.substring(0,4));
    let stmt = ["exec dbo.PROC_JWsaleYearQuery"];
    stmt.push(date+","),
    stmt.push(productType+","),
    stmt.push(positionName+","),
    stmt.push(sUserType+","),
    stmt.push(FEmpID)
    return sqlSvc.sqlK3Query(stmt.join(" "));
  }

  exports.getYearTotal = function(date,productType,positionName,sUserType,FEmpID){
    date = parseInt(date.substring(0,4));
    let stmt = ["exec dbo.PROC_JWsaleYearQueryTotal"];
    stmt.push(date+","),
    stmt.push(productType+","),
    stmt.push(positionName+","),
    stmt.push(sUserType+","),
  ...
  exports.getMonthSaleList = function(date,productType,sUserType,sDeptName,FEmpID){

    let year = parseInt(date.substring(0,4));
    let month = parseInt(date.substring(5,7));
    let stmt = ["exec dbo.PROC_JWsaleQuery_DMS_WEB"];
    stmt.push(year+","),
    stmt.push(month+","),
    stmt.push("终稿版"+","),
    stmt.push(productType+","),
  ...
  exports.getPeopleAndTime = function(date,productType){
    let year = parseInt(date.substring(0,4));
    let month = parseInt(date.substring(5,7));
    var stmt = "select tu.FName as tFname,tj.FCheckTime1,tu_.FName as t_Fname,tj.FCheckTime2 from t_Jwsale tj";
    stmt+="  LEFT JOIN t_SubMessage ts on ts.FInterID = tj.FLB ";
    stmt+="   left join t_user tu on tu.FUserID = tj.FChecker1";
    stmt+="   left join t_user tu_ on tu_.FUserID = tj.FChecker2";
    stmt+="   where tj.FYear = "+year+" and tj.FMonth = "+month+" and tj.FVersion = '终稿版' and ts.FName = '"+productType+"'";
    let paramTypes={};
    let paramValues={};
    return sqlSvc.sqlK3Query(stmt,paramTypes,paramValues);
  } 